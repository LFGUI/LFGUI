...
void setup_sample_gui(lfgui::widget* gui)
{
    ...
    {
        {
            ...
            float r=50;
            float r2=20;
            e.img.draw_polygon(e.depth_buffer,e.depth,
                             {{50+int(sin(0.0*PI)*r),100+int(-cos(0.0*PI)*r)},{50+int(sin(0.2*PI)*r2),100+int(-cos(0.2*PI)*r2)},
        ...
        }
        {
            ...
            float r=50;
            float r2=20;
            e.img.draw_path(e.depth_buffer,e.depth,
                           {{150+int(sin(0.0*PI)*r),100+int(-cos(0.0*PI)*r)},{150+int(sin(0.2*PI)*r2),100+int(-cos(0.2*PI)*r2)},
        ...
        for(int i=10;i<=190;i+=20)
            e.img.draw_line(e.depth_buffer,e.depth,100,100,i,200,{64,64,128},(i+10)/20);

        // draw text centered at the bottom with different alignments
        e.img.draw_text(e.depth_buffer,e.depth,e.img.width()/2,e.img.height()-90,"alignment left",{55,55,0},16,lfgui::alignment::left);
        e.img.draw_text(e.depth_buffer,e.depth,e.img.width()/2,e.img.height()-70,"alignment centered",{55,55,0},16,lfgui::alignment::center);
        e.img.draw_text(e.depth_buffer,e.depth,e.img.width()/2,e.img.height()-50,"alignment right",{55,55,0},16,lfgui::alignment::right);
    });
...

    {
        ...
        {
            e.img.draw_rect(e.depth_buffer,e.depth,e.offset_x,e.offset_y,e.widget.width(),e.widget.height(),{222,222,255,192});
            e.img.draw_text(e.depth_buffer,e.depth,e.offset_x+10,e.offset_y+3,"I move on left and right\nmouse clicks and\nmouse wheel movement",{64,0,0});
        };
    ...
    }
    {
        lfgui::window* window=gui->add_child(new lfgui::window(240,10,270,300,"example window",false,true));
        {
            ...
            {
                e.img.draw_rect(e.depth_buffer,e.depth,e.offset_x,e.offset_y,e.widget.width(),e.widget.height(),{64,64,64});
                e.img.draw_text(e.depth_buffer,e.depth,e.offset_x+10,e.offset_y+3,"I can be dragged",{255,0,255});
            });
...

    {
        ...
        {
            e.img.draw_rect(e.depth_buffer,e.depth,e.offset_x,e.offset_y,e.widget.width(),e.widget.height(),color_background);
            e.img.draw_text(e.depth_buffer,e.depth,e.offset_x+10,e.offset_y+3,"I can be dragged vertically",{255,255,255});
        });
        ...
        {
            e.img.draw_rect(e.depth_buffer,e.depth,e.offset_x,e.offset_y,e.widget.width(),e.widget.height(),{255,255,255});
            e.img.draw_image(e.depth_buffer,e.depth,e.offset_x,e.offset_y,painted_image);
        });
...

    {
        ...
        paint_area->on_paint([paint_area,slider_cos,slider_sin,slider_speed](lfgui::event_paint e)
        {
            ...
            e.img.draw_rect(e.depth_buffer,e.depth,e.offset_x,e.offset_y,e.widget.width(),e.widget.height(),{0,0,0});
            ...
            for(int x=0;x<e.widget.width();x+=stepsize)
            {
                ...
                e.img.draw_line(e.depth_buffer,e.depth,e.offset_x+x,e.offset_y+sin_old*h+h,e.offset_x+x+stepsize,e.offset_y+sin_new*h+h,{192,192,255});
                e.img.draw_line(e.depth_buffer,e.depth,e.offset_x+x,e.offset_y+cos_old*h+h,e.offset_x+x+stepsize,e.offset_y+cos_new*h+h,{192,255,192});
                e.img.draw_line(e.depth_buffer,e.depth,e.offset_x+x,e.offset_y+(sin_old+cos_old)*h+h,e.offset_x+x+stepsize,e.offset_y+(sin_new+cos_new)*h+h,{255,192,192});
