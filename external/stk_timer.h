...
/// @brief A timer class that measures CPU cycles. These are not per process!
/// This is usefull to measure small CPU intense tasks. Uses clocks or timer otherwise.
class timer_cycles
{
    class measure_point
    {
    public:
        std::string text;
        uint64_t tv;
        measure_point(std::string text) : text(text) {__asm__ __volatile__ ("rdtsc" : "=A"(tv));}
    };
...
public:
    /// \brief The text is the general name for this timer. Setting output to false disables printing anything on destruction.
    timer_cycles(std::string text="",bool output=true) : text(text),output(output) {__asm__ __volatile__ ("rdtsc" : "=A"(tv));}
    /// \brief The text is the general name for this timer. Setting output to false disables printing anything on destruction.
    timer_cycles(const char* text,   bool output=true) : text(text),output(output) {__asm__ __volatile__ ("rdtsc" : "=A"(tv));}
    timer_cycles(const timer_cycles&)=default;
    timer_cycles(timer_cycles&&)=default;
    timer_cycles& operator=(const timer_cycles&)=default;
    timer_cycles& operator=(timer_cycles&&)=default;
    ...
    /// \brief Returns the cycles passed since the timer was started or reset.
    uint64_t until_now() const
    {
        clock_t end;
        __asm__ __volatile__ ("rdtsc" : "=A"(end));
        return end-tv;
    ...
    /// \brief Resets the starting cycle count to the current cycle count. Measure points added before this will display a negative count in the summary.
    void reset(){__asm__ __volatile__ ("rdtsc" : "=A"(tv));}
};
